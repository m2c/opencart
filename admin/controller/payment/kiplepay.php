...
class ControllerPaymentkiplepay extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('payment/kiplepay', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('payment/kiplepay', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['kiplepay_merchant'])) {
			$data['kiplepay_merchant'] = $this->request->post['kiplepay_merchant'];
		} else {
			$data['kiplepay_merchant'] = $this->config->get('kiplepay_merchant');
		}

		if (isset($this->request->post['kiplepay_password'])) {
			$data['kiplepay_password'] = $this->request->post['kiplepay_password'];
		} else {
			$data['kiplepay_password'] = $this->config->get('kiplepay_password');
		}

		$data['callback'] = HTTP_CATALOG . 'index.php?route=payment/kiplepay/callback';
		...
		$this->response->setOutput($this->load->view('payment/kiplepay.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'payment/kiplepay')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if (!$this->request->post['kiplepay_merchant']) {
			$this->error['merchant'] = $this->language->get('error_merchant');
		}

		if (!$this->request->post['kiplepay_password']) {
